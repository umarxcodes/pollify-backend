...
class CommentService {
  ...
  async replyToComment(commentId, userId, pollId, content) {
    ...
    const poll = await commentRepository.getPollById(pollId);
    if (!poll) throw new ApiError(404, "Poll not found");
    if (parentComment.pollId.toString() !== pollId.toString()) {
      throw new ApiError(400, "Comment does not belong to this poll");
    }
  ...
  async toggleLike(commentId, userId) {
    ...
    if (existingLike) {
      await commentRepository.decrementLikesCount(commentId);
