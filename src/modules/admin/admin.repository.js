...
class AdminRepository {
  ...
  async getUsers(filters = {}, sort = "newest", page = 1, limit = 20) {
    ...
    const users = await User.find(query)
      .select(
        "name username email role isVerified isSuspended isDeleted deletedAt createdAt lastActive"
      )
      .skip(skip)
      .limit(limit)
      .sort(sortOption);
  ...
  async getUserById(id) {
    return await User.findById(id).select(
      "name username email role isVerified isSuspended isDeleted deletedAt createdAt lastActive loginActivity"
    );
  }
